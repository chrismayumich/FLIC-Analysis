...
SingleWellByDay <- function(wellName, bindfm, starttime, binsize.minutes){
  ...
  for (tIdx in 1:length(timedata)){
    ...
    t <- timedata[tIdx]
    #The timedata is in minutes; each data has 1440 minutes, so Day 2 first row starts at 1440 and so on.
...
BinFeedingData.Licks.PlotWellsByDay <- function(dfm, bindfm, binInterval,title=""){
  ...
  lapply(as.list(1:length(resultsByDay)), plotSingleDay, resultsByDay, colorList, breakList, title)
  return()
}

plotSingleDay <- function(dayNum, resultsByDay, colorList, breakList,title){
  x <- 1:ncol(resultsByDay[[dayNum]])
  y <- 1:nrow(resultsByDay[[dayNum]])
  z <- t(resultsByDay[[dayNum]])
